...
function stencil_diffusion!(dtT, T, k, Cp, ρ, A, z, heat_flux_moho, neumann_bottom)
...
function semi_dicretisation(du,u,p,t)

    @unpack geoparam, grid, thermal_param, physical_prop, magemin_interact = p
    @unpack z, zc = grid
    ...
    # only update if outside the depth of magemin
    # for z in zc
    #     if z < depth_min_thermo || z > depth_max_thermo
    #         Cp .= Cp .+ (dϕdT .* Ql)
    #     end
    # end
    ...
    if convection[1] == true
        ...
        stencil_diffusion!(du, u, k_conv, Cp, ρ, A, z, heat_flux_moho, neumann_bottom)
    elseif convection[1] == false
        stencil_diffusion!(du, u, k, Cp, ρ, A, z, heat_flux_moho, neumann_bottom)
    else
